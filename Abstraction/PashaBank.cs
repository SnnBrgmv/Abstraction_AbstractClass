...
{
    internal class PashaBank : BankCard
    {
        public PashaBank(string fullname, string cardNumber, string cvv, decimal balance) : base(fullname, cardNumber, cvv, balance)
        {}
        public override void Deposit(decimal amount)
        {
            Console.WriteLine("My PashaBank account:\nMy balance: " + Balance + " AZN");
            Console.WriteLine("Deposit money: " + amount + " AZN");
            decimal commission = amount * .006m;
            amount -= commission;
            if (amount > 0)
            {
                Balance += amount;
        ...
        public override void Withdraw(decimal amount)
        {
            ...
            decimal commission = amount * .011m;
            amount -= commission;
            if (amount <= Balance)
            {
                Balance -= amount;
